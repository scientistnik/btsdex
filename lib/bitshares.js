...
import {TransactionBuilder, TransactionHelper, PrivateKey, PublicKey, Login, Aes, ChainStore} from "bitsharesjs"
...
export default class BitShares {
  ...
  async memo(toName, message) {
    ...
    async memoDecode(memos) {
        if (!this.memoKey)
            throw new Error("Not set memoKey!");

        let memo_text;
        let pub_key = await PublicKey.fromPublicKeyString(memos.from);
        let prv_key = this.memoKey;
        // console.log("\nPrivate key:", prv_key.toWif());
        // console.log("Public key :", prv_key.toPublicKey().toString(), "\n");

        if (prv_key) {
            try {
                memo_text = await Aes.decrypt_with_checksum(
                        prv_key,
                        pub_key,
                        memos.nonce,
                        memos.message
  ...
  async transfer(toName, assetSymbol, amount, memo) {
     ...
     * @returns {json object}
     */
    async assetReserve(assetSymbol, amount) {
        await this.initPromise;
        ...
        return this.sendTransaction("asset_reserve",operation);
    }
