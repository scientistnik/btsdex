export default class Event {
  ...
  static connectedNotify() {
    ...
    if (!this.connected.map.all)
      this.connected.map.all = {subs: new Set([undefined]), events:[undefined]}
  ...
  static async subscribeAccount() {
    ...
    Object.keys(this.account.map).forEach(async accName => {
      ...
      if (!obj.history)
        obj.history = (await this.bts.history.get_account_history(obj.id, "1.11.0", 1, "1.11.0"))[0].id;
    })
  ...
  static subscribe(event, callback, accName) {
    ...
    if (event == 'account') {
      this[event].subscribe(callback, accName)
    }
    else
  ...
  static async updateAccounts(ids) {
    for (let id of ids) {
      let acc = this.account.map[(await this.bts.accounts.id(id)).name]

      if (!acc.history)
        acc.history = "1.11.0";

      acc.events = await this.bts.history.get_account_history(id, acc.history, 100, "1.11.0");
  ...
  async subscribe(callback, accName) {
    accName = accName || 'all';
  ...
  notify() {
    Object.keys(this.map).forEach(accName => {
      let handler = this.map[accName];

      handler.subs.forEach(async sub => {
        if (sub && (handler.events.length !=0))
          sub(handler.events)
      })
    })
  }
