...
describe("BitShares class", () => {
  describe("#connect()", () => {
    it("connect", async () => {
      ...
      assert.equal(await connect(process.env.BITSHARES_NODE), true, "Return not 'true'")
  ...
  describe("#login()", () => {
    ...
    it("login", async () => {
      let { login } = BitShares;
      let acc = await login(process.env.BITSHARES_ACCOUNT, process.env.BITSHARES_PASSWORD)
  ...
  describe.skip("#assetIssue()", () => {
    ...
    it("issue asset", async () => {
      let bot = await BitShares.login(process.env.BITSHARES_ACCOUNT, process.env.BITSHARES_PASSWORD);
      let asset = (await bot.balances(process.env.ISSUE_ASSET))[0];
      let balanceBefore = asset.amount / 10 ** asset.asset.precision;
      
      await bot.assetIssue(process.env.BITSHARES_ACCOUNT, process.env.BITSHARES_ISSUE_ASSET, 10, "Hello");

      let balanceAfter = (await bot.balances(process.env.BITSHARES_ISSUE_ASSET))[0].amount / 10 ** asset.asset.precision;
      assert.equal(balanceBefore + 10, balanceAfter, "Asset don't issued")
    });

    it("generateKeys", async () => {
      console.log(BitShares.generateKeys(process.env.BITSHARES_ACCOUNT, process.env.BITSHARES_PASSWORD))
    })
  });
});